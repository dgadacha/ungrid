...
Future<void> main(List<String> args) async {
  ...
  if (args.contains('--from-json')) {
    final raw = jsonDecode(await File('benchmark_stop_tiles.json').readAsString())
  ...
  stdout.writeln('generatorVersion $currentGeneratorVersion, '
  ...
  stdout.writeln('${rows.length} candidats résolus en '
  ...
  stdout.writeln('Écrit : benchmark_stop_tiles.json, '
...
List<Map<String, dynamic>> _distribution(List<List<double>> rows) {
  ...
  return [
    for (final (label, lo, hi) in _buckets)
      () {
        final inBucket =
            rows.where((r) => r[index] >= lo && r[index] < hi).toList();
        return {
          'bucket': label,
          'count': inBucket.length,
          'percentage':
              rows.isEmpty ? 0.0 : inBucket.length / rows.length * 100,
...
Map<String, dynamic> _describe(List<double> row) {
  ...
  final solution =
      level == null ? const <String>[] : const LevelSolver(maxExploredStates: 60000).solve(level).exampleSolution;
  ...
  return {
    ...
    'metrics': {
      for (var i = 0; i < fields.length; i++) fields[i]: row[i],
    },
  };
}
    ...
    'stopTileUsage': {
      ...
      'byTileCount': {
        for (var n = 0; n <= 6; n++)
          '$n': () {
            final rows =
                groupB.where((r) => r[_f('stopTileCount')] == n).toList();
      ...
      'longButEasy': [
        for (final r in (all
                .where((r) =>
                    ...
                    r[_f('difficultyScore')] <= 30)
                .toList()
              ..sort((x, y) => y[_f('moveComplexity')]
                  .compareTo(x[_f('moveComplexity')])))
            .take(10))
          _describe(r),
      ],
      'shortButHard': [
        for (final r in (all
                ...
                .toList()
              ..sort((x, y) =>
                  y[_f('difficultyScore')].compareTo(x[_f('difficultyScore')])))
            .take(10))
          _describe(r),
      ],
    },
...
Future<void> _write(
  ...
  await File('benchmark_stop_tiles.json').writeAsString(jsonEncode({
    'generatorVersion': currentGeneratorVersion,
    'fields': fields,
    'rows': [
      for (final r in rows)
  ...
  await File('benchmark_stop_tiles_summary.json')
      .writeAsString(encoder.convert(summary));

  final all = rows;
  final groupB = rows.where((r) => r[_f('stopTileCount')] > 0).toList();
  await File('benchmark_top_levels.json').writeAsString(encoder.convert({
  ...
  await File('benchmark_stop_tiles_report.md')
...
String _verdict(Map<String, dynamic> s) {
  ...
  if (deepens) {
    buffer.writeln('**Oui.** Les tuiles allongent les solutions '
    ...
    buffer.writeln('**La tuile ajoute de la profondeur sans coûter de '
        ...
        'aucune.');
    buffer.writeln();
    buffer.writeln('Ce qui reste en retrait est l\'étroitesse du chemin optimal '
    ...
    buffer.writeln('**Pas vraiment.** Les tuiles allongent les solutions '
  ...
  buffer.writeln();
  buffer.writeln('${_fmt(meaningful, 1)} % des arrêts changent ce que les '
  ...
  buffer.writeln('Le plafond, lui, a bien sauté. Sans tuile, le rapport tient '
...
String _markdown(Map<String, dynamic> s, List<List<double>> rows) {
  ...
  buffer.writeln();
  buffer.writeln('Generator version ${run['generatorVersion']} · '
      '${run['seeds']} seeds · ${run['candidates']} candidats résolus · '
      '${_fmt(run['elapsedSeconds'], 1)} s');
  buffer.writeln();
  buffer.writeln('Les deux groupes sont construits **sur les mêmes seeds**, '
  ...
  for (final field in _compared) {
    ...
    buffer.writeln('| $field | ${_fmt(ma, 3)} | ${_fmt(mb, 3)} | '
  ...
  buffer.writeln('| candidats de qualité | '
  ...
  for (final entry in ['noStopTile', 'stopTile']) {
    buffer.writeln('### ${entry == 'noStopTile' ? 'Sans tuile' : 'Avec tuile'}');
    buffer.writeln();
    buffer.writeln('| Tranche | Candidats | % | Difficulté | Décision | '
        'Étroitesse | Dépendance | Arrêts | Trivialité |');
    buffer.writeln('| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |');
    for (final bucket
        in (s['distribution'] as Map)[entry] as List<Map<String, dynamic>>) {
      if (bucket['count'] == 0) continue;
      buffer.writeln('| ${bucket['bucket']} | ${bucket['count']} | '
  ...
  buffer.writeln('- arrêts le long de la solution : '
      '${_fmt(usage['avgInteractions'])}');
  buffer.writeln('- dont porteurs d\'une dépendance : '
      '${_fmt(usage['avgMeaningful'])} '
      '(${_fmt((usage['meaningfulShare'] as double) * 100, 1)} %)');
  buffer.writeln('- niveaux avec au moins une tuile inutilisée : '
      '${usage['levelsWithUnusedTile']} '
      '(${_fmt((usage['unusedTileRate'] as double) * 100, 1)} %)');
  buffer.writeln();
  buffer.writeln('| Tuiles | Candidats | Coups/bloc | Difficulté | Trivialité |');
  buffer.writeln('| ---: | ---: | ---: | ---: | ---: |');
  (usage['byTileCount'] as Map).forEach((key, value) {
    if (value == null) return;
    final v = value as Map<String, dynamic>;
    buffer.writeln('| $key | ${v['count']} | '
  ...
  buffer.writeln();
  buffer.writeln('| Grille | Candidats | moyenne (ms) | P95 (ms) | P99 (ms) | '
  ...
  buffer.writeln('**Longs mais faciles** — rapport coups / blocs au-dessus de '
  ...
  buffer.writeln();
  buffer.writeln('**Courts mais durs** — rapport sous 1,20 pour une difficulté '
  ...
  buffer.writeln();
  buffer.writeln('Les dix premiers de chaque catégorie sont détaillés dans '
      '`benchmark_stop_tiles_summary.json`, les meilleurs candidats dans '
      '`benchmark_top_levels.json`.');
  buffer.writeln();
  buffer.writeln('---');
  buffer.writeln();
  buffer.writeln('Fichiers : `benchmark_stop_tiles.json` (détail, '
