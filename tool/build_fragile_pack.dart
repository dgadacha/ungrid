...
void main() {
  ...
  final seen = <String>{};
  for (var seed = 1; seed <= 12000 && candidates.length < 60; seed++) {
    ...
    final level = base.copyWith(
      stopTiles: [],
      fragileStopTiles: base.stopTiles,
    );
    final solved = solver.solve(level);
    if (!solved.solvable) continue;
    final original = solver.solve(base);
    // La disparition doit changer le problème : on exige une économie de
    // coups par rapport aux mêmes arrêts permanents.
    if (!original.solvable || solved.minimumMoves >= original.minimumMoves) {
      continue;
    }
    ...
    if (!engine.isCompleted ||
        engine.activeStopTiles.isNotEmpty ||
        !seen.add(LevelFingerprint.of(level))) {
      continue;
    }
    candidates.add({
      ...level.toJson(),
      ...
      'permanentMoves': original.minimumMoves,
  ...
  Directory('build/fragile').createSync(recursive: true);
  File('build/fragile/fragile_v1.json').writeAsStringSync(
